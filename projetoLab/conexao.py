...
def home():
    ...
    return render_template('index.html')  # Agora usa templates/
...
# Nova rota para a página de avaliação
@app.route('/avaliacao')
def avaliacao():
...
@app.route('/getTable_graficoGeral', methods=['GET'])
def getTable_graficoGeral():
    connection = get_db_connection()
    if connection is None:
        return jsonify({'error': 'Erro de conexão com o banco'}), 500
    try:
